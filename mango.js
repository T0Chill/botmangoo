...
// Авторизация
const gtoken = '5123120664522f3585481aa10103043626c1a2073356dfbb942fc8dbe9f54b8c02883c328494a9649ad60' // токен группы
const ut = 'eb9bd047f1219170bca9bd83c46fc551dfa6f3bab2ea01fd82cf065a30ea886bf4443ede86c68ea08b180' // токен вашей страницы вк получать тут https://vkhost.github.io/ от приложения kate mobile
const gid = '191348968' //ид(группы)
...
[
{name: 'Вип в боте mango', cost: 25000000000, earn: 250000000, workers: 75, id: 11, icon: '🗺'},
{name: 'Модератор в боте mango', cost: 3000000000000, earn: 1000000000, workers: 150, id: 11, icon: '🗺'},
{name: 'Админ в боте mango', cost: 9000000000000, earn: 5000000000, workers: 250, id: 11, icon: '🗺'},
{name: 'MVP в боте mango', cost: 20000000000000, earn: 11000000000, workers: 500, id: 11, icon: '🗺'},
{name: 'Программист в боте mango', cost: 100000000000000, earn: 45000000000, workers: 1000, id: 11, icon: '🗺'},
{name: 'Родня создателю бота mango', cost: 200000000000000, earn: 75000000000, workers: 1500, id: 11, icon: '🗺'}
]];
...
cm.use(async (message, next) => {
	...
	if(/\[club191348968\|(.*)\]/i.test(message.text)) message.text = message.text.replace(/\[club191348968\|(.*)\]/ig, '').trim();
	...
	if(!uid[message.user]){
		...
		message.send(`@id${message.user}(Чел), напиши "регистрация"`)
		message.send({sticker_id:15791})
		acc.users[numm] = {
			lock: true,
			restart: false,
			full: false,
...
setInterval(() => {
	var st = [`📝`,`✏`,`💭`].random();
	var rt = [`Бот: online\n ${st} Принято более ${spaces(acc.msg)} сообщений!`,`Бот: online\n 👥 С нами уже ${spaces(acc.number)} игроков!`,].random();
...
cm.hear(/^(?:токены)/i, message => { 
	if(message.user != 384975224) return
 ...
 return message.send(`
	...
	return message.send(`@id${user.id}(${user.prefix}), новый статус профиля: Закрытый`); // Исполнительный текст
     	   ...
     	   cm.hear(/^(?:ID|Мой ид|мой ID)/i,  (message) => { // Сама команда
     	   	let user = acc.users[user_id(message.user)]; 
     	   	if(user.act == false) return message.send(`@id${user.id}(${user.prefix}), Ваш Аккаунт не активирован!\n — Для активации пиши 'Регистрация'. \n— После чего следуйте дальнейшим инструкциям.`);
	return message.send(`@id${user.id}(${user.prefix}), ID Вашего аккаунта в @botmango (Бот mango): ${user_id(message.user)}\nID Вашего профиля ВКонтакте: @id${user.id}(${user.id})`); // Исполнительный текст
});
...
cm.hear(/^(?:FD)\s?([0-9]+)?/i, (message) => {  
	let user = acc.users[user_id(message.user)];
	if(message.user != 384975224) return;
	    if(!message.$match[1]) return message.send(`Что-то пошло не так 😱\n Подсказка: Пример команды: FD [ID] \n -- FD это - Выдача полного доступа к системе @botmango (Бот mango) `) // Подсказка для команды
...
cm.hear(/^(?:DFD)\s?([0-9]+)?/i, (message) => {  
	let user = acc.users[user_id(message.user)];
	if(message.user != 384975224) return;
	    if(!message.$match[1]) return message.send(`Что-то пошло не так 😱\n Подсказка: Пример команды: DFD [ID] \n -- DFD это - Снятие полного доступа с игрока в системе @botmango (Бот mango) `) // Подсказка для команды
...
cm.hear(/^(?:untiban)\s?([0-9]+)?/i, (message) => {  
	let user = acc.users[user_id(message.user)];
	if(message.user != 384975224) return;
	    if(!message.$match[1]) return message.send(`Что-то пошло не так 😱\n Подсказка: Пример команды: UNTIBAN [ID] \n -- UNTIBAN это - Выдача Анти блокировки игроку в боте: @botmango (Бот mango) `) // Подсказка для команды
...
cm.hear(/^(?:duntiban)\s?([0-9]+)?/i, (message) => {  
	let user = acc.users[user_id(message.user)];
	if(message.user != 384975224) return;
	    if(!message.$match[1]) return message.send(`Что-то пошло не так 😱\n Подсказка: Пример команды: DUNTIBAN [ID] \n -- DUNTIBAN это - Снятие Анти блокировки с и грока в боте: @botmango (Бот mango) `) // Подсказка для команды
...
cm.hear(/^(?:Система)/i,  (message) => { 
	...
	if(message.user != 384975224) return;
	return message.send(`
		🔧 Техническая информация сервера @botmango (Бота Анны) 🔧
...
vk.updates.hear(/^(?:ssh|shel)\s([^]+)$/i, (message) => {
	if(message.user != 384975224) return message.send(`фи`);
	...
	if(!message.$match[1]) return message.send(`Уважаемый @id${user.id} (Основатель), произошла ошибка!\n -- Для того что бы включить или выключить бонус введите: bonus [баланс/опыт] [вкл/выкл]`)
		...
		if(message.$match[1] == 'баланс'){
			if(message.$match[2] == 'вкл'){ config.bonus_balance = true; 
				message.send(`@id${user.id}(${user.prefix}), Успех!\n Вы включили х2 баланс для всех игр/бонусов в @botmango (Бот mango)`); 
			...
			if(message.$match[2] == 'выкл'){ config.bonus_balance = false; 
				message.send(`@id${user.id}(${user.prefix}), Вы выключили х2 баланс для всех игр/бонусов @botmango (Бот mango)`); 
				return message.send({sticker_id:4658});
			}
		}
		if(message.$match[1] == 'опыт'){ 
			if(message.$match[2] == 'вкл'){ config.bonus_exs = true; 
				message.send(`@id${user.id}(${user.prefix}), Успех!\n Вы включили х2 опыт для всех игр/бонусов в @botmango (Бот mango)`); 
				message.send({sticker_id:4649})
				return message.send({attachment:`audio449532928_456239406`});
			} 
			if(message.$match[2] == 'выкл'){ config.bonus_exs = false; 
				message.send(`@id${user.id}(${user.prefix}), Вы выключили х2 опыт для всех игр/бонусов @botmango (Бота mango)`); 
				return message.send({sticker_id:4658});
			}
	...
	if(user.qotkaz == true) return message.send(`${rnick}, вы отказались от пользовательского соглашения.`)
		if(user.qoferta == false){
			message.send(`⚡ Приветствую вас, ${rnick}!
				...
				В статье 3 закона № 152-ФЗ УК-РФ сказано, что оператором является лицо, которое организует и осуществляет обработку персональных данных. Неким лицом является группа: "@botmango (mango | Игровой бот)".
...
vk.updates.hear(/^(?:фермы|🔋 Фермы)\s?([0-9]+)?/i, (message) => {  
	...
	if(!message.$match[1]){
		message.send(`
			...
			🔸 2. GTX 2080 10₿/10 минут (100.000.000$)
			🔹 3. DA2020-BT500 100₿/10 минут (900.000.000$)
	...
	if(i > 0 && i <= 3){
		...
		setTimeout(() => {
			...
			vk.api.call('messages.send', {
				peer_id: user.id,
				message: `[Система @botmango (Бот mango)]: @id${user.id}(${user.prefix}), аренда майнинг-фермы закончилась.\n -- Вы получили ${count[i]}฿.\n\n Если вы решились продать свои биткоины, то вам поможет команда: продать биткоины [Кол-во]`,
				random_id: 0
			});
		}, 600000); 
...
vk.updates.hear(/^(?:Vipe|вайп)\s?([^]+)?/i,  message => {
	let user = acc.users[user_id(message.user)];
	if(message.user != 384975224) return; 
	if(!message.$match[1]) return message.send(`Уважаемый @parsaik (Основатель), Для того что бы произвести вайп введите: Вайп [рейтинга/баланса/администрации/uptime]`);  
	...
	if(message.$match[1] == 'рейтинга'){ 
		...
		message.send(`@id${user.id}(${user.prefix}), Успех!\n Вы Онулировали всем игрокам рейтинг в @botmango (Бота mango)`); 
		message.send({sticker_id:4649})
		return message.send({attachment:`audio449532928_456239406`});
	}
	if(message.$match[1] == 'баланса'){ 
		...
		message.send(`@id${user.id}(${user.prefix}), Успех!\n Вы Онулировали всем игрокам баланс в @botmango (Бота mango)`); 
		message.send({sticker_id:4649})
		return message.send({attachment:`audio449532928_456239406`});
	}
	if(message.$match[1] == 'администрации'){ 
		...
		message.send(`@id${user.id}(${user.prefix}), Успех!\n Вы сняли всю Администрацию @botmango (Бота Анны)`); 
		message.send({sticker_id:4649})
		return message.send({attachment:`audio449532928_456239406`});
	}
	if(message.$match[1] == 'uptime'){ 
		...
		message.send(`@id${user.id}(${user.prefix}), Успех!\n Вы Онулировали счетчик UpTime всем игрока @botmango (Бота Анны)`); 
		message.send({sticker_id:4649})
		return message.send({attachment:`audio449532928_456239406`});
	}
}); 
...
if(!message.$match[1]){
	return message.send(`
		...
		5.💻 » Компьютер Apple Mac Pro - 500.000 $
...
let i = message.$match[1]; 
let ids = [0,1,2,3,4]
let count = [0,50000,120000,190000,250000,500000];
let names = [0,'ATX Midtower Delux DW600 Blac','HyperPC OBSIDIAN','HYPERPC COSMOS 7','HYPERPC COSMOS X','Apple Mac Pro']
if(i < 0 || i > 5) return;
...
// Управление компьютером
      	      vk.updates.hear(/^(?:Компьютер)/i, (message) => { // Сама каоманда
      	      	...
      	      	if(user.act == false) return message.send(`@id${user.id}(${user.prefix}), Ваш Аккаунт не активирован!\n — Для активации пиши 'Регистрация'. \n— После чего следуйте дальнейшим инструкциям.`);
      	      	return message.send(`
💻 >> OnPC - Включить Компьютер
💻 >> OffPC - Выключить компьютер
💻 >> SPC - Состояние Компьютера
 		 	 		 	`); // Исполнительный текст
      	      });
// Включение 
vk.updates.hear(/^(?:OnPC)/i, (message) => { 
	let user = acc.users[user_id(message.user)];
		if(user.pk == false) return message.send(`У вас нет компьютера`); // Если нету компьютера
...
// Выключение
vk.updates.hear(/^(?:OffPC)/i, (message) => { 
	let user = acc.users[user_id(message.user)];
	if(user.pk == false) return message.send(`У вас нет компьютера`); // Если нету компьютера
	...
	return message.send(`@id${user.id}(${user.prefix}), вы успешно выключили свой Компьютер`); // Исполнительный текст
     ...
     vk.updates.hear(/^(?:SPC)/i, (message) => { // Команда
     	let user = acc.users[user_id(message.user)];
 ...
 return message.send(`
   ...
   vk.updates.hear(/^(?:Телефоны|Смартфоны)\s?([1-9]+)?/i, (message) => {  
   	let user = acc.users[user_id(message.user)];
...
if(!message.$match[1]){
	return message.send(`
1.📱 Смартфон Nokia 3310 Blue - 1.000 $
2.📱 Смартфон Samsung Galaxy J1 (2017) 16GB Black - 5.000 $
3.📱 Смартфон Apple IPhone 5s 32Gb Silver (Серебристый) - 7.900 $
4.📱 Смартфон Samsung Galaxy S8 Plus 128Gb - 13.000 $
5.📱 Смартфон Apple iPhone X 256Gb Black (Черный) - 29.000 $
6.📱 Смартфон Apple iPhone XS Max 64Gb Black (Черный) - 55.000 $
7.📱 Смартфон Samsung Galaxy Fold 2 1000Gb Black - 190.000 $
...
let count = [0,1000,5000,7900,13000,29000,55000,190000];
let names = [0,'Nokia 3310','Samsung Galaxy J1','Apple IPhone 5s','Samsung Galaxy S8 Plus', 'Apple iPhone X', 'Apple iPhone XS Max', 'Samsung Galaxy Fold 2']
if(i < 0 || i > 6) return;
	...
	return message.send(`@id${user.id}(${user.prefix}), Вы купили телефон  (${names[i]}) за ${count[i]}$ !\nЧтобы зайти в меню, напишите "phone"\n Советуем вам купить сим-карту "buysim"`)
      	      ...
      	      vk.updates.hear(/^(?:правила)/i, (message) => {
      	      	...
      	      	return message.send(`
      	      		...
      	      		🔞 Наказание: Бан || Предупреждение. 
      	      		🔸 10. Использование БАГов, скрытие их от @parsaik (разработчика)
      	      ...
      	      vk.updates.hear(/^(?:раздача)$/i, async (message) => { 
      	      	...
      	      	let smile = ['🙂','😯','☺','🤑','😉'].random();
      	      	let nsmile = ['🎊','⭐','🔊','🔥','🎮','🌈','💡','💳','🏆','🎉','💸'].random();
      	      	user.api.wall.post({owner_id: -191348968, message: `🏆 Доброго времени суток! Сделайте репост этой записи и получите: ${spaces(bvin)}$ на свой игровой баланс и ${rvin}₽ на свой донат счёт.
      	      		-- Начало акции в ${time()}. Длительность акции 3 часа.`, attachments: 'photo-179018727_456239024'}).then((response) => { 
      	      			user.api.wall.openComments({owner_id: -191348968, post_id: response.post_id });
      	      			user.api.wall.createComment({owner_id: -191348968, post_id: response.post_id, from_group: 191348968, message: '' });
      	      			user.api.wall.createComment({owner_id: -191348968, post_id: response.post_id, from_group: 191348968, message: `*Вы должны быть зарегистрированы в боте, а ваш профиль ВКонтакте открыт.\n-- Валюта будет выдана по покончанию акции, об этом вы будите оповещены в личных сообщениях бота.`});
      	      			user.api.wall.closeComments({owner_id: -191348968, post_id: response.post_id});
      	      			message.send(`Вы успешно запустили раздачу.\n Ссылка на раздачу: [vk.com/wall-191348968_${response.post_id}].`)

      	      			setTimeout(() => {
      	      				user.api.wall.getReposts({owner_id: -191348968, post_id: response.post_id, count: 1000}).then((res) => { 
      	      					res.items.map(x=> { 
      	      						...
      	      						vk.api.messages.send({user_id: 384975224, message: `[Авто-Раздача]: Игроку "(@id${user.id} (${user.prefix})" выдано: ${spaces(bvin)}$ и ${rvin}₽ на баланс.\n\nЕго баланс составляет: ${spaces(user.balance)}$ и ${spaces(user.rub)}₽`}) 
      	      					}); 
      	      				});

      	      				user.api.wall.openComments({owner_id: -191348968, post_id: response.post_id }); 
      	      				user.api.wall.createComment({owner_id: -191348968, post_id: response.post_id, from_group: 191348968, message: '' }); 
      	      				user.api.wall.createComment({owner_id: -191348968, post_id: response.post_id, from_group: 191348968, message: '*Раздача окончена. Всем участникам Акции призы были успешно начисленны.'}); 
      	      				user.api.wall.closeComments({owner_id: -191348968, post_id: response.post_id});
      	      				giving = false; 
      	      			}, 18000000);
      	      		}); 
...
vk.updates.hear(/^(?:!kick|!кик|!кикнуть)(\s?https\:\/\/vk\.com\/)?(id)?([0-9]+)?([^]+)?/i, (message) => {
	...
	if(message.$match[4]) { 
		...
		vk.api.call("utils.resolveScreenName", { 
			...
			if(res.object_id == 384975224) return message.reply('⛔  Произошла критическая ошибка системы\n -- Подсказка: @parsaik (Разработчика) невозможно кикать из бесед!');  
		...
		if(message.$match[3] == 278877039) return message.reply('⛔ Произошла критическая ошибка системы\n -- Подсказка: Главного Администратора @botmango (mango) невозможно кикать из бесед!😡');
		if(message.$match[3] == 384975224) return message.reply('⛔ Произошла критическая ошибка системы\n -- Подсказка: @parsaik (Разработчика) невозможно кикнуть из бесед!'); 
...
vk.updates.hear(/^(?:поиск|search)(\s?https\:\/\/vk\.com\/)?(id)?([0-9]+)?([^]+)?/i, (message) => {
	...
	if(message.$match[3]){
		...
		return message.send(`
			...
			ID Игрока в @botmango (Бот mango): ${id}
...
vk.updates.hear(/^(?:рассылка)\s?([^]+)?/i,  message => { 
	...
	if(message.user != 384975224) return message.send(`⚠ Доступ закрыт ⚠`);
	for(i in acc.users){
		vk.api.call('messages.send', {
...
// Рассылки для беседы
        vk.updates.hear(/^(?:ch)\s?([^]+)?/i,  message => {  // сообщение
        	...
        	if(message.user != 384975224) return message.send(`⚠ Доступ закрыт ⚠`);
        	vk.api.call('messages.send', {
			chat_id: 1, //Беседа Бот mango
			message: `${message.$match[1]}`,
						random_id: 0 // Сообщение
					});
        	return message.send(`Сообщение успешно отправлено в Официальную беседу @botmango (Бот mango)`);
        });
         ...
         vk.updates.hear(/^(?:cp)\s?([^]+)?\s([^]+)?/i, (message) => { // сообщение и пост
         	...
         	if(message.user != 384975224) return message.send(`⚠ Доступ закрыт ⚠`);
         	vk.api.call('messages.send', {
			chat_id: 1, //Беседа Бот 
			message: `${message.$match[1]}\n`, // Сообщение
			attachment: `${message.$match[2]}`,
						random_id: 0 // Вложение
					});
         	return message.send(`Сообщение c вложением успешно отправлено в Официальную беседу @botmango (Бот mango)`);
         });
			...
			attachment: `${message.$match[2]}`,
                	...
                	return message.send(`Музыка успешно отправлена в Официальную беседу @botmango (Бот mango)`);
                });
         ...
         vk.updates.hear(/^(?:ct)\s?([^]+)?/i,  message => { // стикер
			...
			sticker_id: `${message.$match[1]}`,
         	...
         	return message.send(`Стикер успешно отправлен в Официальную беседу @botmango (Бот mango)`);
         });
...
// Рассылки для беседы 
              	        ...
              	        vk.updates.hear(/^(?:МузТест)/i,  (message) => { // Сама команда
              	        	let user = acc.users[user_id(message.user)]; 
              	        	if(message.user != 384975224) return  message.send(`А вот и низя)))`);
              	        	...
              	        	message.send({attachment:`photo-191348968_456240645`})
              	        	message.send({sticker_id: 33});
              	        	message.send(` А вот и обещанная музяка`)
              	        	return message.send({attachment:`audio-191348968_456239145`})
              	        });
	...
	if(user.block_give == true) return message.send(`@id${user.id}(${user.prefix}), вам был заблокирован доступ к передачи средств`)
		...
		}).then((res) => {}).catch((error) => {console.log('[Система]: Александр и Сергей! Я обнаружила ошибку в системе: Перевод денежных средств в команде "передать"'); });	
		return message.send(`@id${user.id}(${user.prefix}),  вы передали игроку ${acc.users[message.$match[1]].prefix} ${message.$match[2]}$. 😉`);
              	        ...
              	        vk.updates.hear(/^(?:giv)\s?([0-9]+)?\s?([^\s  ].*)?/i,  message => {
              	        	...
              	        	adm_log(is)
              	        	return message.send(`💰 @id${user.id}(${user.prefix}), Успех! 😎\n [Система @botmango (Бот mango)]: Зачисляю ироку @id${acc.users[message.$match[1]].id}(${acc.users[message.$match[1]].prefix}) - ${spaces(message.$match[2])}$\n\n Баланс игрока: ${acc.users[message.$match[1]].balance}$`);
...
vk.updates.hear(/^(?:setnick)\s?([0-9]+)?\s([^]+)?/i, (message) => { 
	...
	vk.updates.hear(/^(?:delladmin)\s?([0-9]+)?\s([^]+)?/i, (message) => {  
		...
		if(message.$match[1] == 1) return message.send(`Уважаемый @id${user.id}(Администратор), к сожалению @parsaik (Разработчика) невозможно снять!`);
...
vk.updates.hear(/^(?:CMD)$/i, (message) => { 
	...
	if(user.full == false) return;
	{
		return message.send(`Команды @parsaik (Разработчика)
			👑 Основные:
			...
			37. Прассылка [Идентификатор поста (Пример: Прассылка wall-191348968_1427)] - Создать рассылку поста.
...
vk.updates.hear(/^(?:Стикер)\s?([0-9]+)?/i,  message => {
	...
	message.send({
		sticker_id: stick}).catch((error) => {return message.send(`?? ${rnick}, к сожалению, мой @parsaik (Владелец) не купил мне ещё пак в котором будет стикер №${message.$match[1]}`)});
});
...
vk.updates.hear(/^(?:permban)\s?([0-9]+)?\s([^]+)?/i, (message) => {  
	...
	if(acc.users[message.$match[1]].ban == true) return message.send(`@id${user.id}(${user.prefix}), Что-то пошло не так 😱\n -- Аккаунт игрока ${acc.users[message.$match[1]].prefix} уже имеет перманентную блокировку\n Для разблокировки, используйте: UnPermBan ${message.$match[1]}.`);
	if(!acc.users[message.$match[1]]) return message.send(`@id${user.id}(${user.prefix}), Что-то пошло не так 😱\n Пользователь не обнаружен в Базе данных.\n Подсказка: Возможно вы не правельно ввели ID Пользователя. Пожалуйста, проверьте правильность введенных данных.`);
	if(message.$match[1] == 2) return message.send(`🔸 Уважаемый @id${user.id}(Администратор), к сожалению @parsaik (Разработчику) невозможно выдать временную блокировку.`);
...
vk.updates.hear(/^(?:warn)\s?([0-9]+)?\s([^]+)?/i, (message) => { 
	...
	if(acc.users[message.$match[1]].ban == true) return message.send(`@id${user.id}(${user.prefix}), Что-то пошло не так 😱\n -- Аккаунт игрока ${acc.users[message.$match[1]].prefix} Уже заблокирован навсегда.`);
	if(!acc.users[message.$match[1]]) return message.send(`🔸 @id${user.id}(${user.prefix}), Что-то пошло не так 😱\n Пользователь не обнаружен в Базе данных.\n Подсказка: Возможно вы не правельно ввели ID Пользователя. Пожалуйста, проверьте правильность введенных данных.`);
	if(message.$match[1] == 2) return message.send(`Уважаемый @id${user.id}(Администратор), к сожалению @parsaik (Разработчику) невозможно выдать Предупреждение`);
...
vk.updates.hear(/^(?:AVIG)\s?([0-9]+)?\s([^]+)?/i, (message) => { 
	...
	if(!Number(message.$match[1])) return message.send(`🔸 @id${user.id}(${user.prefix}), Что-то пошло не так 😱\n Подсказка: ID Пользователя должен быть цифрового вида.`);
	if(!acc.users[message.$match[1]]) return message.send(`🔸 @id${user.id}(${user.prefix}), Что-то пошло не так 😱\n Пользователь не обнаружен в Базе данных.\n Подсказка: Возможно вы не правельно ввели ID Пользователя. Пожалуйста, проверьте правильность введенных данных.`);
	if(message.$match[1] == 2) return message.send(`Уважаемый @id${user.id}(Основатель), к сожалению @parsaik (Разработчику) невозможно выдать выговор`);
...
vk.updates.hear(/^(?:unban)\s?([0-9]+)?/i, (message) => { 
	...
	vk.api.call('messages.send', {
		peer_id: acc.users[message.$match[1]].id,
		message: `[Система @botmango (Бот mango)]: С вашего аккаунта досрочно была снята временная блокировка. \n -- Больше не нарушайте :)`,			random_id: 0
	});
...
vk.updates.hear(/^(?:unpermban)\s?([0-9]+)?/i, (message) => { 
	...
	vk.api.call('messages.send', {
		peer_id: acc.users[message.$match[1]].id,
		message: `[Система @botmango (Бот mango)]: С вашего аккаунта была снята перманентная блокировка.\n -- Больше не нарушай :)`,			random_id: 0
	});
	var is = [user_id(message.user), message.text] 
	adm_log(is)
...
vk.updates.hear(/^(?:unwarn)\s?([0-9]+)?/i, (message) => { 
	...
	vk.api.call('messages.send', {
		peer_id: acc.users[message.$match[1]].id,
		message: `[Система @botmango (Бот mango)]: С вашего аккаунта были сняты все предупреждения.\n -- Больше не нарушай :)`,			random_id: 0
	});
	var is = [user_id(message.user), message.text] 
	adm_log(is)
...
vk.updates.hear(/^(?:unavig)\s?([0-9]+)?/i, (message) => { 
	...
	vk.api.call('messages.send', {
		peer_id: acc.users[message.$match[1]].id,
		message: `[Система @botmango (Бот mango)]: С вашего аккаунта были сняты все Административные выговора.\n -- Больше не нарушай :)`,			random_id: 0
	});
	var is = [user_id(message.user), message.text] 
	adm_log(is)
...
vk.updates.hear(/^(?:ungban)\s?([0-9]+)?/i, (message) => { 
	...
	vk.api.call('messages.send', {
		peer_id: acc.users[message.$match[1]].id,
		message: `[Система @botmango (Бот mango)]: Вам была разблокированна возможность выдачи валюты. [giv]\n -- Больше не нарушайте :)`,			random_id: 0
	});
	var is = [user_id(message.user), message.text] 
	adm_log(is)
...
vk.updates.hear(/^(?:unrban)\s?([0-9]+)?/i, (message) => { 
	...
	vk.api.call('messages.send', {
		peer_id: acc.users[message.$match[1]].id,
		message: `[Система @botmango (Бот mango)]: Вам была разблокированна возможность писать жалобы в репорт.\n -- Больше не нарушайте :)`,			random_id: 0
	});
	var is = [user_id(message.user), message.text] 
	adm_log(is)
...
vk.updates.hear(/^(?:sex|секс|заняться сексом)\s?([0-9]+)?/i, (message) => { 
	...
	if(message.$match[1] == 1) return message.send(`Уважаемый @id${user.id}(пользователь), к сожалению эту команду нельзя использовать на @parsaik (Разработчике)!`);
	var is = [user_id(message.user), message.text] 
	user.block_por = true;
	setTimeout(() => {
...
vk.updates.hear(/^(?:kiss|поцеловать)\s?([0-9]+)?/i, (message) => { 
	...
	if(user.level < 1) return message.send(`🔸 ➾ Вы не VIP-Пользователь`);
	if(!acc.users[message.$match[1]]) return message.send(`❎ ➾ Игрок не найден!`);
	if(message.$match[1] == 2) return message.send(`Уважаемый @id${user.id}(пользователь), к сожалению эту команду нельзя использовать на @parsaik (Разработчике)!`);
	var is = [user_id(message.user), message.text] 
	user.block_porn = true;
	setTimeout(() => {
		user.block_porn = false;
...
vk.updates.hear(/^(?:Бот)/i,  (message) => {
	...
	if(user.act == false) return message.send(`[Чат-Менеджер @botmango (Бот mango)]: ${rnick}, Ваш Аккаунт не активирован!\n — Для активации пиши 'Регистрация'. \n— После чего следуйте дальнейшим инструкциям.`);
...
vk.updates.hear(/^(?:о боте)$/i, (message) => {
	...
	return message.send(`
		...
		📚 Прочее: 
		⛔ » Не забудьте вступить в группу: @botmango (Бот mango)
...
vk.updates.hear(/^(?:беседы|беседа)$/i,  (message) => { 
	let user = acc.users[user_id(message.user)];
	return message.send(`
		@id${user.id}(${user.prefix}), Ссылки на наши офф.беседы:\n - https://vk.me/join/AJQ1d9WocxYaosfwyBDobNa_`);
});
...
vk.updates.hear(/^(?:developer)/i,  message => {
	...
	message.send(`
		...
		📊 » Фамилия: Андрианов 📌
		📑 » Отчество: Сергеевич📌
		📑 » Дата рождения: 15.09.2005 📌
		📑 » Проживание: Г. Кострома 📌

		⚙ » Боты Серёжи: 
		⚙ 1  @botmango (mango | Игровой Бот) 'Основной'

		ᅠ          ❤ » Интересы: Программирование, изучение, создание
		ᅠ          📡 » Связь: https://vk.com/parsaik || @parsaik (Сергей Андрианов)
		💻 » ID Профиля во всех его ботах: 0 или 1
		💻 » ID В данном боте: 1
		💻 » Уровень доступа: Основатель
		💻 » ID VK: @parsaik  (384975224 )
		...
		if(!message.$match[1]){
			...
			return message.send(`
				...
				`+(id.verify == false ? `⛔ Верификация: не подтверждён\n` : `✓ Подтверждёный Профиль.\n
					Эта отметка означает, что профиль ${id.prefix} подтвержден администрацией @botmango (Бот mango).\n\n`)+
...
vk.updates.hear(/^(?:лог)/i, (message) => {
 ...
 vk.updates.hear(/^(?:донат)/i,  message => {
 	let user = acc.users[user_id(message.user)];
 	return message.send(`	
 		💳Донат услуги проекта @botmango (Бот mango)💳
 		...
 		🔻 Срок: Навсегда >> 3000₽. -- Покупка у @parsaik (Основателя)

 		🔹 >> Уровень Доступа: Full-Dostup
 		🔻 Срок: Навсегда >> 5000₽.  -- Покупка у @parsaik (Основателя)
...
vk.updates.hear(/^(?:setpromo)\s?([0-9]+)?/i, message => {
 ...
 cm.hear(/^(?:full)\s?([0-9]+)?\s?([0-9]+)?/i,  (message) => { 	 
 	if (message.user != 384975224) {
 		message.reply('Только для Разработчика.');
 		return message.sendSticker(16701);
 	}
 ...
 vk.updates.hear(/^(?:питомцы)\s?([0-9]+)?/i, message => {
   ...
   🐲15. Дракон. 200.000 $
 	...
 	let i = message.$match[1];
 	let user = acc.users[user_id(message.user)];  
     let count = [0, 500,1000,1450,3000,5000,8000,10000,15000,25000,30000,50000,70000,100000,150000,200000];
 	let names = [0,'Совунья','Крош','Бараш','Пандик','Коала','Попугай','Пудель','Акула','Ястреб','Леопард','Змея','Павлин','Пингвин','Tyrannosaurus rex','Дракон']
 	if(i < 0 || i > 15) return;
 	...
 	if(i > 0 && i <= 15){
 		...
 		user.balance -= count[i];
 		user.pit = names[i];
 		message.send(`🐼 ➾ Вы купили питомца (${names[i]}) за ${count[i]}$`)
         return message.sendSticker(10879);
 	}
 ...
 vk.updates.hear(/^(?:Продать питомца)/i,  message => {
 	...
 	user.pit = false;
 	message.send(`🐼 ➾ Вы успешно продали питомца.`);
     return message.sendSticker(16700);
 });
 ///////////////////////////////////////////////////////
...
//------------------------------------------------------------------------------------\\
 ...
 vk.updates.hear(/^(?:nakruntka)/i, message => {
 	...
 	message.send(`Накрутка комментариев успешно зпущена\n -- Пост на который идёт накрутка  | https://vk.com/wall384975224_1428 |`)
 });
...
vk.updates.hear(/^(?:skin 256)/i,  (message) => { // Сама команда
	...
	let rnick = (user.anick ? `@id${user.id}(${user.prefix})` : `${user.prefix}`)
	if(message.user != 384975224) return;
	message.send(`@parsaik (Артём) вы сменили свой скин на ID: 256`,{attachment:`photo-179018727_456239018`});
...
vk.updates.hear(/^(?:!vkNick)/i,  (message) => {
	...
	vk.api.call('users.get', {user_ids: message.user}).then(res => {let user = res[0]; 
		acc.users[user_id(message.user)].prefix = `${user.first_name} ${user.last_name}`;});
	return message.send(`[Чат-Менеджер @botmango (Бот mango)]: ${rnick}, вы успешно установили себе ник по данным вашей @id${user.id} (странице ВКонтакте)`);
});

vk.updates.hear(/^(?:Рандмуз)/i,  (message) => {
	let user = acc.users[user_id(message.user)];
	let rnick = (user.anick ? `@id${user.id}(${user.prefix})` : `${user.prefix}`)
	let stick = [2796,2797,10873,10880,16689].random();
...
vk.updates.hear(/^(?:РандСтик)/i,  (message) => {
	let user = acc.users[user_id(message.user)];
	let rnick = (user.anick ? `@id${user.id}(${user.prefix})` : `${user.prefix}`)
	let stick = [2796,2797,10873,10880,16689,15800,15805, 16128].random();
	if(user.act == false) return message.send(`${rnick}, Ваш Аккаунт не активирован!\n — Для активации пиши 'Регистрация'. \n— После чего следуйте дальнейшим инструкциям.`);
	message.send({sticker_id: stick});
});
...
vk.updates.hear(/^(?:!tkick|!ткик)\s?([0-9]+)?\s([0-9]+)?\s([^]+)?/i, (message) => { 	
	...
	if(id == 384975224) return message.reply('err Unknown del users development ⛔');
	vk.api.call("messages.send", {
		chat_id: ch,
...
cm.hear(/^(?:time)/i, message => {
	return message.send(`Точное время у @parsaik(Сереги): \n----------------------
		${time(1)} (МСК)
		`);
});
...
cm.hear(/^(?:группа)/i, (message) => {
	...
	return message.send(`[botmango|Ссылка на нашу группу: ]`, {
		attachment: 'photo-191348968_456239099'
	});
});
...
vk.updates.hear(/^(?:!setname)\s([^]+)?/i, (message) => { 
	let user = acc.users[user_id(message.user)];
	if(user.full == false) return message.send(`[Чат-Менеджер Бот mango]: ${rnick}, Ваш Аккаунт не активирован!\n — Для активации пиши 'Регистрация'. \n— После чего следуйте дальнейшим инструкциям.`); 
	if(message.$from.type == 'user') return message.send(`[Чат-Менеджер Бот mango]: ⛔ Произошла критическая ошибка системы\n — Подсказка:команда работает только в беседах!`);
	if(user.full == false) return; 
	vk.api.call("messages.editChat", {chat_id: message.chatId, title: message.$match[1]}).catch((error) => {return message.send(`[Чат-Менеджер Бот mango]: ⛔ ${rnick}, произошла критическая ошибка системы\nВозможные причины:\n Сообщество не имет достаточно доступа, сообщество не Администратор беседы.`);}); 
	message.send(`[Чат-Менеджер Бот mango]: Название беседы было успешно изменено на: ${message.$match[1]}`, {attachment: `audio449532928_456239452`}); 
